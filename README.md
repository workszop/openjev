...
For deployment, any static HTTPS host is sufficient. The live copy is on GitHub Pages at https://workszop.github.io/openjev/. Pages cannot send the COOP/COEP headers from `_headers`, so the page is not cross-origin isolated there and wllama loads the model single-threaded (about 20 s from cache instead of 3 s on the test machine); inference runs on WebGPU either way and results are identical. Hosts that honour `_headers` (Cloudflare Pages, Netlify) get the faster load. Locally, `python3 serve.py` sends the headers. No build step, API, database, telemetry, or server-side inference is used. wllama 3.6.1 is vendored; Vue and Material Symbols load from pinned CDN URLs.
